...
%
% Sprawdzaczke nalezy uruchomic w katalogu w którym znajduja sie testy
% i rozwiazania:
%
% $ swipl prac2.pl
%
% Predykat test_all/0 uruchamia rozwiazanie na wszystkich testach
% is wyswietla raport. np.
%
% ?- test_all.
...
%
% Predykaty test_all_resolve/0 oraz test_all_prove/0 dzialaja podobnie,
% z ta róznica, ze uruchamiaja tylko testy dla predykatu odpowiednio
% resolve/4 oraz prove/2
%
% Za pomoca predykatu run_test/1 mozna uruchomic pojedynczy test podajac
% jego nazwe:
%
% ?- run_test(excluded_middle).
% excluded_middle                0.003s ok
%
% Dla kazdego uruchomionego testu sprawdzaczka wyswietla czas dzialania
% (o ile test sie w~pelni wykonal) oraz status wykonania. Mozliwe wartosci
% to:
%
% ok             - program przeszedl test
% wrong answer   - bledna odpowiedz
% tle            - przekroczony limit czasu
% invalid test   - niepoprawny format testu
% invalid answer - niepoprawna format rozwiazania. W przypdaku predykatu
%     resolve/4 moze oznaczac, ze predykat zakonczyl sie wiecej niz
%     jednym sukcesem
%
% Sprawdzaczka udostepnia jeszcze dwa predykty, ulatwiajace odpluskwianie
% kodu:
%
% show_proof(+Input, +Proof) - wyswietla dowód Proof (np. rezultat dzialania
%   predykatu prove/2) w czytelnej postaci. Input powinno byc lista aksjomatów
%
% test_and_show(+Name) - uruchania predykat prove/2 na tescie o nazwie Name
%   i wyswietla wyprodukowany dowód za pomoca show_proof
%
% UWAGA: niniejsza sprawdzaczka nie sprawdza wszystkich warunków nalozonych
% na format testów i wyników dzialania programu. Zachecamy jednak do
% modyfikowania i ulepszania jej kodu.
%
% Do poprawnego dzialania sprawdzaczki nalezy jeszcze zmodyfikowac
% ponizsze dwa wiersze:
:- use_module(adam_kufel_tests).
:- use_module(adam_kufel).
